      ...
      pattern when is_binary(pattern) ->
        case Regex.compile(pattern) do
          ...
          {:error, reason} ->
            :ok =
              Logger.warn(
                "[libring] ignore_node?/3: invalid blacklist pattern (#{inspect(pattern)}): #{
                  inspect(reason)
                }"
              )
      ...
      pattern when is_binary(pattern) ->
        case Regex.compile(pattern) do
          ...
          {:error, reason} ->
            :ok =
              Logger.warn(
                "[libring] ignore_node?/3: invalid whitelist pattern (#{inspect(pattern)}): #{
                  inspect(reason)
                }"
              )
